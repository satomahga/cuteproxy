  ...
  if (GATEKEEPER === "user_token" && token) {
    ...
    switch (result) {
      ...
      case "suspended": {
        ...
        const until = (u as any)?.meta?.suspendUntil as number | undefined;
        const reason = (u as any)?.meta?.suspendReason || "User token temporarily suspended";
        const msg = until ? `${reason} until ${new Date(until).toISOString()}` : reason;
        return sendError(req, res, 403, `Forbidden: ${msg}`);
      }
      case "disabled":
